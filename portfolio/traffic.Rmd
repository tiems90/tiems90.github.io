...
```{r}
#Rename columns so they make more intuitive sense
traffic <- traffic0 %>% 
  mutate(
...
#Below 0 values. Something is off.
...
The documentation also metions that in our measuring points data, there is a factor that indicates urban or interurban positions. Let's update these factorlabels. 
...
df %>% 
  ...
  select(7:10) %>% 
  cor(use='complete') %>% 
  corrplot('square', main='City Roads')

df %>% 
  filter(tipo_elem == 'Highway') %>% 
  select(7:11, -10) %>% 
  cor(use='complete') %>% 
  corrplot('square', main='Highway')
```
...
Now, after reading the documentation and seeing the `animation_tmap()` function, this sounds like something interesting to do! In order to keep my computer alive I will average by the hour though.
...
remove.id <- id.df %>% filter(N!= median(id.df$N))
